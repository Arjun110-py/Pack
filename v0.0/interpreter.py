...
class Interpreter:
    ...
    def visit_BinOpNode(self, node):
        ...
        if node.op_tok.type == TT_PLUS: res = left.added_to(right)
        elif node.op_tok.type == TT_MINUS: res = left.subbed_by(right)
        elif node.op_tok.type == TT_MUL: res = left.multed_by(right)
        elif node.op_tok.type == TT_DIV: res = left.dived_by(right)
        else: res = self.no_visit_method(self, node)
...
def run(path):
    ...
    for line in read_file(path).split("\n"):
        ...
        print(interpreter.visit(ast.node))
